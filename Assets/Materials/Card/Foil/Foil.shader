	...
	SubShader
	{
		...
		CGPROGRAM
			...
			fixed4 frag(v2f IN  ) : SV_Target
			{
				...
				float4 color106 = IsGammaSpace() ? float4(0,0.525753,1,1) : float4(0,0.2386567,1,1);
				float4 lerpResult110 = lerp( saturate( ( staticSwitch2_g1 + ( ( 1.0 - saturate( min( abs( dotResult74 ) , 0.9 ) ) ) / 2.0 ) ) ) , color106 , 0.1);
				
				fixed4 c = saturate( lerpResult110 );
				c.rgb *= c.a;
				return c;
			}
		ENDCG
...
Node;AmplifyShaderEditor.ColorNode;106;2280.978,747.6547;Inherit;False;Constant;_HolographicTint;Holographic Tint;2;0;Create;True;0;0;0;False;0;False;0,0.525753,1,1;0,0,0,0;True;0;5;COLOR;0;FLOAT;1;FLOAT;2;FLOAT;3;FLOAT;4
